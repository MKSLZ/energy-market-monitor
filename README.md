...
- **区域现货（确定性接口）**：纽卡斯尔国际煤价（Trading Economics）、德国日前现货电价（energy-charts / SMARD，取近 24 小时均值）
- **区域现货（新闻提取）**：TTF、秦皇岛 Q5500、印尼 HBA 等从当期新闻文本正则提取并附原文；提取到的报价写入 `data/spot_state.json`，TTL（10 天）内跨期沿用并标注日期，避免某 3 小时窗口无报价时量化归零
...
5. **国内电价专项推演**：把油价 / 气价 / 煤价与事件量化映射到中国电价
   ...
   - 煤价→度电燃料成本：供电煤耗约 300 克/度、长协煤约 80% 对冲，分别测边际煤机成本与综合上网电量成本推力（分/千瓦时）；无官方秦港价时以国际煤价定方向并给假设煤价档位情景（不伪造实测价）
